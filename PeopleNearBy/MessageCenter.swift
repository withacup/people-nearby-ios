...
class MessageCenter {
    ...
    private let FILE_NAME: String = "MessageCenter.swift"
...
//                Debug.printEvent(withEventDescription: "\(contact.date)", inFile: "MessageCenter.swift")
                ...
                // TODO: when sign up and sign in, store user image into coredata, because we need to retrieve that data later
//                let userImage = CoredataService.getImage(withId: userName)
                
                _contacts[contactId] = Contact(withContactName: contactId, contactImg: nil)
