...
class MapVC: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate {
    ...
    private let FILE_NAME = "MapVC.swift"
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        if annotation.isKind(of: MKUserLocation.self) {
            ...
            if let eventAnnotation = annotation as? EventAnnotation {
                ...
                annotationView?.image = eventAnnotation.eventInfo.eventImg
                
                let rightBtn = UIButton(type: UIButtonType.detailDisclosure)
                
                annotationView?.rightCalloutAccessoryView = rightBtn
                annotationView?.leftCalloutAccessoryView = UIImageView(image: eventAnnotation.eventInfo.eventImg)
            }
    ...
    func handleSingleTap(gesture: UIGestureRecognizer) {
        ...
        Debug.printEvent(withEventDescription: "handling new event on location: \(coord)", inFile: FILE_NAME)
            ...
            if let key = key, let location = location {
                ...
                self.REF_EVET.child(key).observeSingleEvent(of: .value, with: { (snapshot) in
                    ...
                    if let value = snapshot.value as? Dictionary<String, String> {
                        ...
                        FIRStorageService.sharedInstance.retrieveEventImage(withId: key, completion: { (eventImage) in
                            
                            let singleEvent = EventInfo(eventName: value["name"]!, eventTime: value["time"]!, eventHolder: value["holder"]!, eventDesription: value["content"]!, eventImg: eventImage, eventId: key)
