...
class CoredataService {
    ...
    private init () {}
    static let FILE_NAME: String = "CoredataService.swift"
...
//        newMess.toContact = currentContact // this is a inverse relationship
    ...
    // MARK: - Insert new image into coredata
    /// This method will insert a new image into context with corresponding id
    static func insert(withNewImage newImage: UIImage, id: String) {
        
        Debug.printEvent(withEventDescription: "storing new image with id: \(id) into coredata", inFile: FILE_NAME)
        
        let newImageData = Images(context: CoredataService.getContext())
        newImageData.imageData = UIImageJPEGRepresentation(newImage, 1.0) as NSData?
        newImageData.imageId = id
    ...
    static func getImage(withId imageId: String) -> UIImage{
        ...
        do {
            images = try CoredataService.getContext().fetch(fetchRequest)
        }catch {
